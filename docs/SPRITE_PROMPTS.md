...
6. **Stabilize (สำคัญ!)**: `npm run stabilize` — art ที่ gen ทีละเฟรมจะมีตำแหน่ง/ขนาดเพี้ยนเฟรมละนิด ทำให้ตัวสั่น/หดพองตอนเล่น สคริปต์นี้จัดกึ่งกลาง ตรึงเท้าลง baseline เดียวกัน normalize ขนาดต่อเฟรม และคำนวณ `scale` ต่อท่าลง meta.json ให้อัตโนมัติ
