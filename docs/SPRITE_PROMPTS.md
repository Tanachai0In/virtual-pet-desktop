...
6. **Stabilize (สำคัญ!)**: `npm run stabilize` — art ที่ gen ทีละเฟรมจะมีตำแหน่ง/ขนาดเพี้ยนเฟรมละนิด ทำให้ตัวสั่น/หดพองตอนเล่น สคริปต์นี้จัดกึ่งกลาง ตรึงเท้าลง baseline เดียวกัน และ normalize ขนาดทั้งในแต่ละเฟรมและข้ามทุกท่าลงใน `sheet.png` โดยตรง (ไม่ใช้ runtime `scale` ต่อท่า)
7. **ตรวจ**: `npm run sheets` — error เรื่องเฟรมโปร่งใส/ขนาด ให้กลับไปข้อ 1-3
8. **เช็คเท้าตรง baseline**: เปิดแอปดู ถ้าตัวลอย/จมพื้น ให้ปรับ `anchor.y` ใน `meta.json` (ค่ามาตรฐาน 120)
